...
export function KeyboardCustomizer({
  ...
  // Push interactive props to parent when using external viewer
  useEffect(() => {
  ...
  }, [externalViewer, onInteractivePropsChange, mergedConfig, selectionMode, selectedKeys, paintMode, handleKeySelect, handleKeyPaint]);

  // Clear interactive props on unmount when using external viewer
  useEffect(() => {
    if (!externalViewer || !onInteractivePropsChange) return;
    return () => {
  ...
  return (
    <div className={cn(
      "flex gap-4 h-full",
      externalViewer ? "flex-col" : "flex-col lg:flex-row",
      className,
    )}>
      {/* 3D Viewer — only when NOT using external viewer */}
      {!externalViewer && (
      ...
      {/* Selection toolbar — inline when using external viewer */}
      {externalViewer && (
      ...
      <div className={cn(
        "shrink-0",
        externalViewer ? "w-full" : "w-full lg:w-[300px] xl:w-[340px]",
      )}>
        <div className={cn(
          "p-4 rounded-2xl bg-bg-surface/80 backdrop-blur-md border border-border-subtle/50 shadow-surface overflow-hidden flex flex-col",
          externalViewer ? "max-h-none" : "h-full max-h-[500px] lg:max-h-[calc(100vh-12rem)]",
        )}>
