...
const ORDERED_SIZES = ["40%", "60%", "65%", "75%", "TKL", "96%", "1800", "100%"];
const EXCLUDED_SIZES = new Set(["numpad", "Unknown"]);
...
export function KeyboardKitPicker({ selected, onSelect }: KeyboardKitPickerProps) {
  ...
  const validKeyboards = useMemo(() => {
    if (!keyboards) return [];
    return keyboards.filter((kb) => !EXCLUDED_SIZES.has(kb.size));
  }, [keyboards]);

  const filtered = useMemo(() => {
    return validKeyboards.filter((kb) => {
      if (search) {
  ...
  }, [validKeyboards, search, sizeFilter, hotSwapFilter, wirelessFilter]);

  const availableSizes = useMemo(() => {
    if (!validKeyboards.length) return [];
    const present = new Set(validKeyboards.map((k) => k.size));
    return ORDERED_SIZES.filter((s) => present.has(s));
  }, [validKeyboards]);
    ...
    <div className="space-y-4">
      <FilterBar
        ...
        filters={[
          {
            label: "Size",
            chips: availableSizes.map((s) => ({ label: s, value: s, active: sizeFilter === s })),
