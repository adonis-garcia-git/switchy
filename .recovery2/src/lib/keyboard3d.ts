...
export interface KeyboardViewerConfig {
  ...
  plateColor: string;
  caseMaterial: "aluminum" | "aluminum-matte" | "polycarbonate" | "plastic" | "wood" | "brass";
  ...
  paintMode?: boolean;
  // ─── Phase 10: Wired/Wireless ──
  wireless?: boolean;
}

const MATERIAL_KEYWORDS: Record<string, KeyboardViewerConfig["caseMaterial"]> = {
  "matte aluminum": "aluminum-matte",
  "matte aluminium": "aluminum-matte",
  matte: "aluminum-matte",
  anodized: "aluminum-matte",
  "e-white": "aluminum-matte",
  cerakote: "aluminum-matte",
  aluminum: "aluminum",
  aluminium: "aluminum",
  alu: "aluminum",
...
export function buildDataToViewerConfig(build: BuildData, keyboard?: KeyboardData): KeyboardViewerConfig {
  ...
  if (keyboard) {
    ...
    // Pass wireless flag
    if ("wireless" in keyboard) {
      config.wireless = !!(keyboard as unknown as Record<string, unknown>).wireless;
    }
  }
