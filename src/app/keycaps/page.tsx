...
import { KeycapFilterBar } from "@/components/KeycapFilterBar";
...
function ActiveFilterBadges({
  ...
  setFilters: (f: KeycapFilterState) => void;
}) {
  const badges: { key: string; label: string; onDismiss: () => void }[] = [];

  if (filters.profile) {
    badges.push({ key: "profile", label: `Profile: ${filters.profile}`, onDismiss: () => setFilters({ ...filters, profile: null }) });
  }
  if (filters.material) {
    badges.push({ key: "material", label: `Material: ${filters.material}`, onDismiss: () => setFilters({ ...filters, material: null }) });
  }
  if (filters.brand) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-4">
      ...
      <button
        onClick={() => setFilters({ ...DEFAULT_KEYCAP_FILTERS })}
...
function KeycapsContent() {
  const [filters, setFiltersRaw] = useFilterParams<KeycapFilterState>(
    DEFAULT_KEYCAP_FILTERS,
    parseKeycapParams,
    keycapFiltersToParams
  );
    ...
    <div className="min-h-screen">
      <div className="flex">
        ...
        {sidebarOpen && (
          <>
            ...
            <aside className="fixed inset-y-0 left-0 w-72 bg-bg-surface border-r border-border-default z-50 lg:hidden overflow-y-auto p-5 pt-20">
              ...
              <KeycapFilterBar filters={filters} onChange={setFilters} brands={brands} />
            </aside>
          </>
        )}
        ...
        <main className="flex-1 min-w-0 px-4 lg:px-8 py-6">
          ...
          <div className="mb-6">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search keycaps by name or brand..."
          ...
          {/* Sponsored carousel */}
          <SponsoredCarousel productType="keycaps" />
          ...
          {mergedItems === null ? (
            ...
            <div className="text-center py-20">
              <p className="text-text-muted text-sm">No keycaps match your filters.</p>
              ...
              <div className={gridClassName}>
                {(() => {
                  ...
                  return mergedItems!.map((item: any) => {
                    ...
                    return (
                      <KeycapCard key={item._id} keycap={item} featured={isFirst} />
                    );
                  });
                })()}
              </div>
...
export default function KeycapsPage() {
  return (
    <Suspense fallback={<KeycapsLoading />}>
      <KeycapsContent />
    </Suspense>
  );
}
