...
import { DEFAULT_ACCESSORY_FILTERS } from "@/lib/filterParams";

interface AccessoryFilterBarProps {
  filters: AccessoryFilterState;
  onChange: (filters: AccessoryFilterState) => void;
  brands: string[];
}

export function AccessoryFilterBar({ filters, onChange, brands }: AccessoryFilterBarProps) {
  const update = (partial: Partial<AccessoryFilterState>) => {
  ...
  return (
    <div className="space-y-0">
      <div className="flex items-center justify-between pb-3 mb-1 border-b border-border-subtle">
        ...
        {hasActiveFilters && (
          <button
            onClick={() => onChange({ ...DEFAULT_ACCESSORY_FILTERS })}
      ...
      <FilterSection title="Category">
        <RadioOption label="All Categories" active={filters.subcategory === null} onClick={() => update({ subcategory: null })} />
        {ACCESSORY_SUBCATEGORIES.map((cat) => (
          <RadioOption
            key={cat.value}
            label={cat.label}
            active={filters.subcategory === cat.value}
            onClick={() => update({ subcategory: filters.subcategory === cat.value ? null : cat.value })}
          />
        ))}
      </FilterSection>
      ...
      <FilterSection title="Price">
        <RangeSlider label="Min" min={0} max={150} step={5} value={filters.minPrice ?? 0} onChange={(val) => update({ minPrice: val > 0 ? val : null })} unit="$" />
        <RangeSlider label="Max" min={0} max={150} step={5} value={filters.maxPrice ?? 150} onChange={(val) => update({ maxPrice: val < 150 ? val : null })} unit="$" />
      </FilterSection>
